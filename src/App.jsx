...
function App() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setError(false)
      try {
        const res = await fetch(`https://pokeapi.co/api/v2/pokemon?limit=${itemsPerPage}`)
  ...
  const toggleFavorite = (pokemon) => {
    setFavorites(prev => {
      ...
      const updated = isFav ? prev.filter(f => f.id !== pokemon.id) : [...prev, pokemon]
          ...
          <Routes>
            <Route path="/" element={
              <>
                ...
                <div className="mt-6 flex flex-col sm:flex-row justify-between gap-4 items-center">
                  ...
                  <div className="flex flex-col sm:flex-row gap-4">
                    <div className="flex flex-col">
                      <label htmlFor="sort" className="text-sm text-gray-700 mb-1">Sort by</label>
                      <select
                        id="sort"
                        value={sortBy}
                    ...
                    <div className="flex flex-col">
                      <label htmlFor="itemsPerPage" className="text-sm text-gray-700 mb-1">Pokémons per page</label>
                      <select
                        id="itemsPerPage"
                        value={itemsPerPage}
                ...
                {loading && <p className="text-center mt-20 text-gray-700 font-medium">Loading Pokémons...</p>}
                {error && <p className="text-center mt-20 text-red-500 font-medium">Failed to load data.</p>}
                {!loading && !error && sortedPokemons.length === 0 && (
                  <p className="text-center mt-20 text-gray-700 font-medium">No Pokémon found.</p>
                )}
